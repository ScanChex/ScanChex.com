...
<?php if ( get_option('chameleon_quote') == 'on' ) { ?>
	<div id="category-name"> 
		...
		<!-- <a href="http://giveyourcomputeradayjob.com/scanchextest/solutions/"><img src="http://giveyourcomputeradayjob.com/scanchextest/wp-content/uploads/2013/03/wide-qr.png"></a> -->
		<?php if ( get_option('chameleon_quote_one') <> '' ) { ?>
			<h6><i>"<?php echo esc_html(get_option('chameleon_quote_one')); ?>"</i></h6>
		<?php } ?>
		<?php if ( get_option('chameleon_quote_two') <> '' ) { ?>
			<p><br><?php echo esc_html(get_option('chameleon_quote_two')); ?></p>
		<?php } ?>	
        </div>
				...
				<?php for ($i=1; $i <= 3; $i++) { ?>
					<?php query_posts('page_id=' . get_pageId(html_entity_decode(get_option('chameleon_home_page_'.$i)))); while (have_posts()) : the_post(); ?>
						...
						<div class="service<?php if ( $i == 3 ) echo ' last'; ?>">
							<h3 align=center class="title"><?php the_title(); ?></h3>
							...
							<?php if ( $thumb <> '' ) { ?>
								<div class="thumb">
									<a href=" <?php echo get_post_meta($post->ID,'etlink',true) ?> ">

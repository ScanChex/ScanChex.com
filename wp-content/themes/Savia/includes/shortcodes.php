...
function shortcode_feat_text( $atts, $content = null ) {
	...
	$content = '<div class="section_featured_texts type'.$type.' animationStart"><div class="icon_holder"><div class="icon_bgr"></div><div class="icon_center"><i class="icon '.$atts['icon'].'"></i></div></div><h3>'.($add_link ? $link : '').$atts['title'].($add_link ? '</a>' : '').'</h3><p>'.do_shortcode_boc($content).'</p></div>';
	return $content;
}
...
function shortcode_posts_carousel($atts, $content = null) {
	...
	if ( $wp_query->have_posts() ):
	    ...
	    while( $wp_query->have_posts() ) : $wp_query->the_post();
			...
			$str .='<li class="'.$width.' '.($heading_bottom_margin ? 'margined_top' : '').'">';
			$id = $wp_query->post->ID;
			...
			$str .= ($show_read_more ? '<a href="'. get_permalink().'" class=\'more-link\'>'.__('Read more','Savia').'</a>' : '');			
			$str .= ($show_date ? '</div>' : '');
...
function shortcode_portfolio_carousel($atts, $content = null) {
		...
		extract(shortcode_atts(
			array(
				'heading' => __("Portfolio Items", "Savia"),
				'display_type' => 1,
